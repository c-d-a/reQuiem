...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	while ((hresult = iDirectSoundCreate(NULL, &pDS, NULL)) != DS_OK)
	{
		...
		{
			Con_Print ("DirectSoundCreate failure, hardware already in use\n");
			return SIS_NOTAVAIL;
		}
...
//			Con_Print ("Using secondary sound buffer\n");
	...
	else
	{
		...
		pDSBuf = pDSPBuf;
//joe		Con_Print ("Using primary sound buffer\n");
	}
...
qboolean SNDDMA_InitWav (void)
{
	...
	if ((hr = waveOutOpen((LPHWAVEOUT)&hWaveOut, WAVE_MAPPER, &format, 0, 0L, CALLBACK_NULL)) != MMSYSERR_NOERROR)
	{
		if (hr == MMSYSERR_ALLOCATED)
			Con_Print ("waveOutOpen failed, hardware already in use\n");
		else
...
// waveOut sound
	...
	{
		if (snd_firsttime)
			Con_Print ("No sound device initialized\n");
...
byte * SNDDMA_LockBuffer (DWORD *dwSize)
{
	...
	if (pDSBuf)
	{
	// JDH: GetStatus/Restore/Play was originally only done in S_Update_
		...
		while ((hresult = pDSBuf->lpVtbl->Lock(pDSBuf, 0, gSndBufSize, (LPVOID *) &pbuf, dwSize, (LPVOID *) &pbuf2, &dwSize2, 0)) != DS_OK)
		{
			if (hresult != DSERR_BUFFERLOST)
			{
				Con_Print ("SNDDMA_LockBuffer: DS::Lock Sound Buffer Failed\n");
				S_Shutdown ();
				S_Startup ();		// JDH: this wasn't done in Lock code in S_ClearBuffer.  Is this OK?
				return NULL;
			}

			if (++reps > 10000)
			{
				Con_Print ("SNDDMA_LockBuffer: DS: couldn't restore buffer\n");
				S_Shutdown ();
				S_Startup ();
				return NULL;
			}
